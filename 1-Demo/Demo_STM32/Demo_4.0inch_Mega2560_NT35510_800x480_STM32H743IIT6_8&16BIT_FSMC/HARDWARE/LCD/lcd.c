...
//       DB5          接          PE8            |
//       DB6          接          PE9            |
//       DB7          接          PE10           |==>>液晶屏16位并口数据信号(8为模式使用DB0~DB7)
//       DB8          接          PE11           | 
//       DB9          接          PE12           |
//       DB10         接          PE13           |
//       DB11         接          PE14           |
...
//    FMC_NORSRAM_TimingTypeDef FSMC_WriteTim;
		...
		GPIO_Initure.Pin=GPIO_PIN_8;            //PB5,背光控制 PB8,复位
  ...
  SRAM_Handler.Init.MemoryDataWidth=FMC_NORSRAM_MEM_BUS_WIDTH_8; //8位数据宽度
    ...
    //FSMC写时序控制寄存器
    FWT.AddressSetupTime=15;        //地址建立时间(ADDSET)为21个HCLK=96ns
    FWT.AddressHoldTime=0x0;
    FWT.DataSetupTime=0x25;           //数据保存时间(DATAST)为4.6ns*21个HCLK=96ns
